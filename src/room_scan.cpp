...
void callback(const sensor_msgs::LaserScan::ConstPtr& msg)
{
    ...
    if (abs(abstand) <= 0.15 || (msg->ranges[180] <= 0.2 && msg->ranges[180] > 0))
    {
    	//std::cout << "Ausweichen" << std::endl;
        driveToWall = false;
        shouldDodge = true;
    }
    ...
    if (turned >= 25)
    {
        for (int i = 0; i <= 359; i++)
        {
            if(distance > msg->ranges[i] && msg->ranges[i] > 0)
            {
        ...
        if ((degree <= 190 && degree >= 180) || (degree <= 179 && degree >= 170))
        {
            driveToWall = true;
        }
    }
